...
module.exports = {
		...
		FB.getLoginStatus((e)=> {
			...
			switch(e.status)
			{
				...
				case 'connected':
					...
					this.pic = 'http://graph.facebook.com/'+ this.id +'/picture?type=large';
					this.token = e.authResponse.accessToken;
					var u = `https://graph.facebook.com/me?access_token=${ e.authResponse.accessToken }`;
					$.get(u, (e) => {
						this.name = e.name;
						if (Hash.get('code')) this.ready({ userID: this.id, name: this.name, accessToken: this.token, imageUrl: this.pic });
					});
				break;
	...
	{
		if(!this.is) { console.log('init first'); return; };
		window.location.href = "https://www.facebook.com/dialog/oauth?client_id=" + this.id + "&redirect_uri=" + encodeURIComponent(redirect);
	},
}
