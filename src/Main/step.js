...
export default class step extends React.Component {
	...
	componentDidMount() {
		$(this.refs.ico).children('div').click((e)=>{
	...
	appendIcon()
	{
		...
		for(var i = 0; i < 10; i++)
		{
			op.push(<div id={ 'p' + i } key={i} class={ 'i ' + 'p'+ i }><div class="txt">{ ReactHtmlParser(Data[i].db[0].name) }</div></div>)
		}
		return op;
	}
	...
	appendSetps()
	{
		...
		for(var i = 0; i < 6; i++)
		{
			op.push(<div key={i} class={ 's ' + 'bg'+ i }><div class={ 'd m' + i }></div></div>)
		}
		return op;
	}
	...
	render() {
		return ( 
			<div id='step'>
				...
				<div class='rows mt-4'>
					<div ref='btn' class='btn'>點擊觀看影片</div>
				</div>
				{ this.appendPopup() }
			</div>
		);
